...
class DBProxy:
    ...
    def _connectConfigMySQL(self):
        ...
        if hasattr(panda_config, 'dbhostmysql'):
            self.dbhost = panda_config.dbhostmysql
        if hasattr(panda_config, 'dbpasswdmysql'):
            self.dbpasswd = panda_config.dbpasswdmysql
        if hasattr(panda_config, 'dbusermysql'):
            self.dbuser = panda_config.dbusermysql
        if hasattr(panda_config, 'dbnamemysql'):
            self.dbname = panda_config.dbnamemysql
            self.schemanamebase = panda_config.dbnamemysql
            self.schemanamemeta = panda_config.dbnamemysql
            self.schemanamegris = panda_config.dbnamemysql
            self.schemanamearch = panda_config.dbnamemysql
        if hasattr(panda_config, 'dbportmysql'):
            self.dbport = panda_config.dbportmysql
...
###            sqlFile += "RETURNING attemptNr,maxAttempt INTO :newAttemptNr,:maxAttempt "
    ...
    def _rollback(self, useOtherError=False):
        ...
        try:
            ...
            rollbackBackend = { \
                'oracle': self._rollbackOracleErrorCodes, \
                'mysql': self._rollbackMySQLErrorCodes \
            }
            # get error codes for a backend
            if self.backend in rollbackBackend.keys():
                error_Codes = rollbackBackend[self.backend](useOtherError)
            else:
                error_Codes = connectBackend['oracle'](useOtherError)
    ...
    # Oracle-specific error codes for _rollback
    def _rollbackOracleErrorCodes(self, useOtherError=False):
        # error codes for connection error
    ...
    # MySQL-specific error codes for _rollback
    def _rollbackMySQLErrorCodes(self, useOtherError=False):
        # error codes for connection error
...
#                      'ORA-01089',  # ## Immediate shutdown in progress - no operations are permitted RACLE not available
                       SERVER_SHUTDOWN,
